...
namespace Ten2Five.Plugins
{
	...
	public static class ExerciseSelection
	{
		...
		private static int GetAccuracy(ExerciseMap w)
		{
		...
		private static int GetTotal(IEnumerable<ExerciseMap> words, out int upper)
		{
			...
			int total = 0;
			int count = 0;
			foreach (ExerciseMap w in words)
			{
				int a = GetAccuracy(w);
				if (a > upper)
					upper = a;
		...
		private static int GetLikelihood(ExerciseMap w, int upper)
		{
			return upper - GetAccuracy(w) + 1;
		}

		public static ExerciseMap Select(IEnumerable<ExerciseMap> words)
		{
			int upper;
